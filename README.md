...
页面打开后会自动加载默认图 `G-default.gv`（38 节点 / 44 边），
...
标签条上方的 `层级 8/8` 表示当前显示 8 层 / 共 8 层。加载新图时默认走「合适层级」。
...
| 文件 | 规模 | 配套节点信息 |
| --- | --- | --- |
| `G-default.gv` | 38 节点 / 44 边（默认图） | 无 |
| `graphs/PHIRE/G0-default.gv` | 与 `G-default.gv` 内容相同 | 无 |
...
- 打开超大图后**耐心等一次**，别反复点按钮。
- **调整 M 会重跑层级计算**（约 7 s/次），大图上尽量少动 M。
...
app/
  ...
  graph-tab-state-store.js     每个标签页的视口、层级、SVG 缓存
...
- 大图（万级节点）解析与层级计算会阻塞主线程约 10 秒以上。
- `graphviz-app.js` 里引用了 `renderModeSelect` 等在 `index.html` 中并不存在的元素，
  因此「概览模式」渲染策略目前**无法从界面触发**（相关代码是历史遗留）。
- `app.js` 是早期基于 `vis-network` 的旧版实现，**已不再被任何页面加载**。
- `HANDOFF.md` 写于文件搬到仓库根目录之前，里面的 `设计中/` 路径已经失效，
  当前有效的路径说明以本文件和 `ARCHITECTURE.md` 为准。
